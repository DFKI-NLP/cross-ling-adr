...
class Augmentor(object):
    ...
    def get_abstractive_summarization(self, text):
        ...
        if self.device.type == "cuda":
            tokenized_text = self.tokenizer.encode(
                t5_prepared_text, return_tensors=self.return_tensors
            ).to(self.device)
        else:
            tokenized_text = self.tokenizer.encode(
                t5_prepared_text, return_tensors=self.return_tensors
    ...
    def process_abstractive_summarization(self, feature, num):
        ...
        df_feature = self.df[
            (self.df[feature] == 1) & (self.df[self.features].sum(axis=1) == 1)
        ]
...
def main():
    ...
    df_augmented.to_csv(
        csv.replace(".csv", "-augmented.csv"), encoding="utf-8", index=False
    )
